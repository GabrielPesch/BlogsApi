...
describe('services/userService', () => {
  ...
  describe('remove', () => {
    it('Deve disparar um erro caso User.destroy também dispare', () => {
      sinon.stub(User, 'destroy').rejects();
      return chai.expect(usersService.remove(1)).to.eventually.be.rejected;
    });
    it('Deve retornar UNDEFINED caso o User.destroy seja bem sucedido', () => {
      sinon.stub(User, 'destroy').resolves();
      return chai.expect(usersService.remove(1)).to.eventually.be.undefined;
    });
  });
});